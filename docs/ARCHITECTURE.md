...
Row 1's merge is write-side only: build_silver still does a full `SELECT * FROM bronze.events` every call, because bronze's upsert-by-`event_id` resets `_ingested_at` on every land (even for byte-identical redeliveries), leaving no safe watermark to filter the read without touching kit-owned bronze code — production scale would need a bronze-side watermark or CDC feed. Row 2's null-`sequence` tie-break is a candidate judgment call, not a kit rule (only null-`event_time`-never-wins is contractual, per `docs/STATUS_GPV_EFFECT.md`). No FX or cross-currency GPV blending was attempted (out of scope per contract).
